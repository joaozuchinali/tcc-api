...
const sqlUpdate = 'UPDATE `usuario` SET `nome` = ?, `senha` = ? WHERE `usuario`.`idusuario` = ?';
const sqlReturn = 'SELECT * FROM `usuario` WHERE `usuario`.`idusuario` = ?';

// http://localhost:12005/api/usuarios/update/
// https://joaozucchinalighislandi.com.br/api/usuarios/update/
router.put('/', async function(req, res) {
    const body = req.body;
    
    if(body.nome && body.senha && body.idusuario) {
        const values = [
            body.nome,
            body.senha,
            body.idusuario
        ...
        const empty = funcs.returnAbsentProps(body, [ 'nome', 'senha', 'idusuario' ]);
        res.status(300).send({
