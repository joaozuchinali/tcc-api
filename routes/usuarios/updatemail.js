...
const sqlUpdate = 'UPDATE `usuario` SET `email` = ? WHERE `usuario`.`idusuario` = ?';
const sqlReturn = 'SELECT * FROM `usuario` WHERE `usuario`.`idusuario` = ?';

// http://localhost:12005/api/usuarios/updatemail/
// https://joaozucchinalighislandi.com.br/api/usuarios/updatemail/
router.put('/', async function(req, res) {
    const body = req.body;
    
    if(body.email && body.idusuario) {
        const values = [
            body.email,
            body.idusuario
        ];
        ...
        const empty = funcs.returnAbsentProps(body, [ 'email', 'idusuario' ]);
        res.status(300).send({
